...
// board constructor, every game init board
  ...
  while (true) {
    ...
    while (true) {
      tuple<bool, int, int, int>
          move_case; // bool true is system command in first int else move as
                     // [pos, tar, prom], prom default is -1
      if (draw_board) {
        draw(1);
      }
...
// updating piece positions, needed
...
// simplicity pre-function for real make_move function
// takes triple -> makes 3 variables to call make move
void Board::make_move(tuple<int, int, int> move) {
  make_move(get<0>(move), get<1>(move), get<2>(move));
}

// make move function handles every move
// updates pieces
void Board::make_move(int old, int target, int promotion) {
  ...
  Move add;
  if (obj.get_type() == 5 && abs(target % 8 - old % 8) > 1) { // castles
    if (target - old == 2) {
      rook_old = old / 8 * 8 + 7;
      rook_new = old / 8 * 8 + 5;
    ...
    add = {3,        old,
           ...
           obj,      rook_obj};
    /*add.type = 3; add.pos = old; add.tar = target; add.rook_pos = rook_old;
    add.rook_tar = rook_new; add.obj = obj; add.rook_obj = rook_obj;
    add.en_passant = -1; add.castles[0] = castles[0]; add.castles[1] =
    castles[1]; add.castles[2] = castles[2]; add.castles[3] = castles[3];*/
    ...
    add = {2,          old,
           ...
           obj,        {},
           tar_obj};
    // add.type = 2; add.pos = old; add.tar = target; add.obj = obj; add.tar_obj
    // = tar_obj; add.en_passant = en_passant;

  } else if (piece_exists[target]) { // capture
    ...
    add = {1,      old,
           ...
           obj,    {},
           tar_obj};
    // add.type = 5; add.pos = old; add.tar = target; add.obj = obj; add.tar_obj
    // = tar_obj;
...
// undo functions undoes move
// necessary for minmax-algorithm
void Board::undo() {
  undo_count++;
  move_count--;
  ...
  int old_pos = last_move.pos, tar_pos = last_move.tar,
      rook_old_pos = last_move.rook_pos, rook_tar_pos = last_move.rook_tar,
      promotion_type = last_move.promotion;
  Piece obj = last_move.obj, tar_obj = last_move.tar_obj,
      ...
      for (int move:*(piece_moves+l)) { // go through every line
        ...
        if (!pin_stopped) {
          if (same_kind == 0) {
            ...
            else {own_color_in_pins = true; pins.push_back(move);}
          }
        }
      }
      if (king_found) {
...
int Board::update_moves() {
  ...
  if (!move_list.empty()) {
    en_passant = -1;
  } // preserve enpassant state from fen
  int udc = (int)move_list.size() - undo_count;
  bool heavy_piece = false, local_en_passant = false, long_possible = false,
       short_possible = false, check = false, any_moves = false;
  ...
  if (castles[(1 - to_play) * 2 + 1]) {
    if (piece_exists[(7 - to_play * 7) * 8]) {
      ...
      if (obj.get_type() == 3 &&
          obj.get_color() ==
              to_play) { // assumable, just in case castles is wrong in fen
        long_possible = true;
...
Piece *Board::get_pl() { return &piece_list[0]; }
bool *Board::get_pe() { return &piece_exists[0]; }
// returns Forsyth–Edwards Notation

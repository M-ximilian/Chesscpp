...
public:
    ...
    // 0-5 für Bauer, Springer, Läufer, Turm, Dame, Koenig
    Piece();
    [[nodiscard]] int get_color() const; // getter Funktionen
    [[nodiscard]] int get_type() const;


    bool pos_in_real_moves(const short int &); // prüft Anwesenheit eines Feldes in der jeweiligen Liste der Figur
    ...
    void set_real_moves(const vector<int>& new_real_moves); // set für Überschreiben
    void add_real_moves(vector<int> new_real_moves); // add für hinzufügen
    void pop_real_moves(const vector<int>&); // pop um zu entfernen

private:
    int color, type;// 0-5, p, n, b, r, q, k
    // legal moves für alle Züge ohne Pins, en passant, rochade
    // real moves für alle erlaubten Züge
    // pins für die Sichtlinie, in der sich der Konig befindet
    // view für alle Züge ohne Hindernisserkennung nach Richtung sortiert
    // legal defend wie legal_moves + erste eigene Figur jeder Sichtlinie, für Bauern nur Diagonalzüge
    // update wie legal defend, für Bauern aber alle Züge
...
struct Position {
    vector<int> position; // Vektor aller Positionen + Rochaderechte
...
static char convert_piece_type(int color, int piece) {
    switch (piece) {
        ...
        case 2:
            return (char) (98 - 32*color); //Läufer
        case 3:
